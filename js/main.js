...
function drawWordWithMouseDistortion(word, x, y) {
    ...
    // Calcular la distancia entre la palabra y el ratón
    const distance = Distance(mouseX, mouseY, wordX, wordY);
    const intensity = DistanceBrightness(distance); // Intensidad proporcional a la distancia

    // Calcular los ángulos de inclinación hacia el ratón
    const deltaX = mouseX - wordX;
    const deltaY = mouseY - wordY;
    const angleX = deltaY / maxDistance; // Inclinación vertical proporcional
    const angleY = deltaX / maxDistance; // Inclinación horizontal proporcional

    // Profundidad simulada: escala inversamente proporcional a la distancia
    const scale = 1 + intensity * 0.3; // Aumenta el tamaño con la intensidad

    // Aplicar transformaciones
    ctx.translate(wordX, wordY); // Centrar las transformaciones en la palabra
    ctx.transform(1, angleX * intensity, angleY * intensity, 1, 0, 0);

    ctx.fillStyle = "rgba(255, 255, 255, " + (intensity*.3) + ")"; // Transparencia basada en la intensidad
    ctx.translate(-wordX, -wordY);
    ctx.translate(deltaX*intensity*.2, deltaY*intensity*.2);
